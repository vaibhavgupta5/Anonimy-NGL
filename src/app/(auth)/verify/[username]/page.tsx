...
function VerifyAccount() {
  ...
  const onSubmit = async (data: z.infer<typeof verifySchema>) => {
    try {
      ...
      const result = await axios.post(`/api/verifycode`, {
                ...
                <FormItem className="text-center flex flex-col justify-center items-center">
                  ...
                  <FormControl>
                    <InputOTP maxLength={6} {...field}>
                      <InputOTPGroup>
                        <InputOTPSlot
                          className="bg-[#0D1117] border-[1px] border-solid border-white text-white"
                          index={0}
                        />
                        <InputOTPSlot
                          className="bg-[#0D1117] border-[1px] text-white border-solid border-white"
                          index={1}
                        />
                        <InputOTPSlot
                          className="bg-[#0D1117] border-[1px] text-white border-solid border-white"
